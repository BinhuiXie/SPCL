...
def get_threshold(cfg):
    ...
    logger.info("Start inference on target dataset and get pseudo label threshold")
    ...
    # 1 epoch data, the whole source and target
    target_train_data = build_dataset(cfg, mode='train', is_source=False, epochwise=True)

    # drop_last=False
    target_train_loader = torch.utils.data.DataLoader(target_train_data,
                                                      ...
                                                      sampler=None,
                                                      drop_last=False)
    objective_vectors = {k: None for k in range(cfg.MODEL.NUM_CLASSES)}

    cpseudo_label = PseudoLabel(cfg)
    for batch in tqdm(target_train_loader):
        x, y, name = batch
        src_input = x.cuda(non_blocking=True)
        src_size = src_input.shape[-2:]
        with torch.no_grad():
            output = classifier(feature_extractor(src_input))
        output = F.interpolate(output, size=src_size, mode='bilinear', align_corners=True)
